...
def names_to_wikipedia(names):
    ...
    for name in names:
        wikipedia_snippit.append(" American multinational technology company based in Seattle, Washington, which focuses on e-commerce, cloud computing, digital streaming, and artificial intelligence.")
    df = pd.DataFrame(
        {
            'names':names,
...
def lambda_handler(event, context):
    ...
    #'eventSourceARN': 'arn:aws:sqs:ap-northeast-1:105372338271:producer'
    #event_source_arn = event['Records'][0]['eventSourceARN']
    event_source_arn = 'arn:aws:sqs:ap-northeast-1:105372338271:producer'
    ...
    for record in event['Records']:
        ...
        res = delete_sqs_msg(queue_name="producer", receipt_handle=receipt_handle)
    ...
    # Write result to S3
    write_s3(df=df, bucket="lambdalayer202003221", name=names)
